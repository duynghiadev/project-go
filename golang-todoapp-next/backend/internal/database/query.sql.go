...
const updateTodos = `-- name: UpdateTodos :exec
UPDATE todos
  set title = $2,
  content = $3
WHERE id = $1
...
func (q *Queries) UpdateTodos(ctx context.Context, arg UpdateTodosParams) error {
	_, err := q.db.ExecContext(ctx, updateTodos, arg.ID, arg.Title, arg.Content)
	return err
}
